...
def _stabilize_box_track(boxes, confs, max_interp_gap: int = 5, alpha: float = 0.65):
    """Interpolate short gaps and bidirectionally smooth an offline xyxy track."""
    boxes = boxes.copy()
    confs = confs.copy()
    observed = np.isfinite(boxes[:, 0])
    ...
    for left, right in zip(ids[:-1], ids[1:]):
        ...
        if 0 < gap <= max_interp_gap:
            for offset in range(1, gap + 1):
                w = offset / (gap + 1)
                boxes[left + offset] = (1 - w) * boxes[left] + w * boxes[right]
                confs[left + offset] = 0.5 * min(confs[left], confs[right])

    tracked = np.isfinite(boxes[:, 0])
    starts = np.flatnonzero(tracked & np.r_[True, ~tracked[:-1]])
    ends = np.flatnonzero(tracked & np.r_[~tracked[1:], True])
    for start, end in zip(starts, ends):
        segment = boxes[start:end + 1]
        ...
        boxes[start:end + 1] = 0.5 * (forward + backward)
    return boxes, confs, observed
...
class OnlineHaMeRPipeline:
    ...
    def process_sequence(
        ...
        if stabilize_boxes and valid_fi:
            track_boxes = np.full((T, 4), np.nan, dtype=np.float32)
            track_confs = np.zeros(T, dtype=np.float32)
            for vi, fi in enumerate(valid_fi):
                track_boxes[fi] = valid_bboxes[vi][0]
                track_confs[fi] = valid_confs[vi]
            track_boxes, track_confs, _ = _stabilize_box_track(
                track_boxes, track_confs, max_interp_gap=max_interp_gap,
            )
            valid_fi = np.flatnonzero(np.isfinite(track_boxes[:, 0])).tolist()
            valid_bboxes = [track_boxes[fi:fi + 1] for fi in valid_fi]
            valid_confs = [float(track_confs[fi]) for fi in valid_fi]
            valid_hw = [raw_images_bgr[fi].shape[:2] for fi in valid_fi]
            valid_sides = [target_side] * len(valid_fi)
        ...
        landmarks_original_all = np.zeros((T, 21, 2), dtype=np.float32)
        ...
        hamer_pose_all = np.zeros((T, 48), dtype=np.float32)
        hamer_betas_all = np.zeros((T, 10), dtype=np.float32)
        hamer_cam_t_all = np.zeros((T, 3), dtype=np.float32)
        ...
        if len(valid_fi) > 0:
            ...
            for s in range(0, len(crops_t), self.hamer_batch_size):
                ...
                global_R = out["pred_mano_params"]["global_orient"].detach().cpu().numpy()
                hand_R = out["pred_mano_params"]["hand_pose"].detach().cpu().numpy()
                hamer_betas = out["pred_mano_params"]["betas"].detach().cpu().numpy()
                ...
                for bi in range(e - s):
                    ...
                    landmarks_original_all[fi] = kp_n_orig
                    ...
                    hamer_pose_all[fi, :3] = cv2.Rodrigues(global_R[bi, 0])[0].ravel()
                    for ji in range(15):
                        hamer_pose_all[fi, 3 + 3 * ji:6 + 3 * ji] = \
                            cv2.Rodrigues(hand_R[bi, ji])[0].ravel()
                    hamer_betas_all[fi] = hamer_betas[bi]
                    hamer_cam_t_all[fi] = cam_t_full[bi]
        ...
        return {
            ...
            "hamer_landmarks_original": torch.from_numpy(landmarks_original_all),
            "crop_intrinsics": torch.from_numpy(crop_intrinsics_all),
            "hamer_confidence": torch.from_numpy(confidence_all),
            "hamer_pose": torch.from_numpy(hamer_pose_all),
            "hamer_betas": torch.from_numpy(hamer_betas_all),
            "hamer_cam_t": torch.from_numpy(hamer_cam_t_all),
