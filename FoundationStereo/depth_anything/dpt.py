...
class DPT_DINOv2(nn.Module):
    def __init__(self, encoder='vitl', features=256, out_channels=[256, 512, 1024, 1024], use_bn=False, use_clstoken=False, pretrained_dino=False):
        ...
        # FoundationStereo vendors DINOv2 and its own checkpoint contains the
        # backbone state. Loading the local hub keeps inference deterministic
        # and avoids cloning facebookresearch/dinov2 at model construction.
        local_dinov2 = os.path.abspath(os.path.join(code_dir, '..', 'dinov2'))
        self.pretrained = torch.hub.load(
            local_dinov2, 'dinov2_{:}14'.format(encoder), source='local',
            pretrained=False,
        )
